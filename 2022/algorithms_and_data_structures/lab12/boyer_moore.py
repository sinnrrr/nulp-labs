"""
1) Boyer-Moore is a smarter naive exact matching algorithm, that skips
unnecessary checks. Has two rules that you need to follow.

2) Bad character rule:

                  |------------------
       A B C D A [B] C D C A (text) |
       A B C D C [A] (pattern)      |
    |----^--------------------------|
    |    |_____|
    |       |
    |     searching for b in here
    |     found here -|
    ------------------|
                      |
    moving to match this letter
                      |
       A B C D A B C D C [A] (text)
               A B C D C [A] (pattern)
               |___________|
...
2) Good suffix rule:
                  ...
                  |-------------------|
       A B C D A [B] C D A B C (text) | the bad charater rule
       B C D A B [C] (pattern)        | allows to skip 1 alignment
             |------------------------|
             |
             |     |------------------|
             |   __|__                |
             |   |   |                |
  |--- A B C D A [B C] D A B C (text) |
  |      B C D A [B C] (pattern)      |
  |     |___|                         |
  |       |                           |
  |       |---------------------------|
