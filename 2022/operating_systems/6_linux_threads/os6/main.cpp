...
int duration;
int sched_policy = SCHED_RR;  // round robing sched policy
int sleep_time;
...
void *_run_find_array_sum(void *) {
  ...
  for (int i = 0; i < thread_ids.size(); i++) {
    pthread_join(thread_ids[i],  // thread descriptor
                 NULL            // don't need thread return result
    );
  }
...
void option_find_array_sum(bool with_sleep) {
  ...
  for (int from = 0, to = batch_size; from < numbers.size();
    ...
    pthread_create(&thread_id,       // thread descriptor
                   &attrs,           // thread attributes (stack size, priority)
                   count_array_sum,  // function to call
